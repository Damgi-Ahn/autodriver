...
{
  const Eigen::Vector4d q(q_map_from_base_.w(), q_map_from_base_.x(),
    q_map_from_base_.y(), q_map_from_base_.z());
  return p_map_.allFinite() && v_map_.allFinite() && q.allFinite() &&
         b_g_.allFinite() && b_a_.allFinite() && P_.allFinite();
}
...
void EskfCore::reset()
{
  ...
  p_map_.setZero();
  v_map_.setZero();
  q_map_from_base_ = Eigen::Quaterniond::Identity();
  b_g_.setZero();
  b_a_.setZero();
  P_.setZero();
  r_eff_inflate_gnss_pos_ = 1.0;
  r_eff_inflate_gnss_vel_ = 1.0;
  r_eff_inflate_heading_yaw_ = 1.0;
}
  ...
  const Eigen::Quaterniond & q_map_from_base)
{
  ...
  p_map_ = p_map;
  v_map_.setZero();
  q_map_from_base_ = q_map_from_base.normalized();
  b_g_.setZero();
  b_a_.setZero();

  P_.setZero();
  P_.block<3, 3>(0, 0).diagonal().setConstant(params_.init_pos_var);
  P_.block<3, 3>(3, 3).diagonal().setConstant(params_.init_vel_var);
  P_.block<3, 3>(6, 6).diagonal().setConstant(params_.init_att_var);
  P_.block<3, 3>(9, 9).diagonal().setConstant(params_.init_bg_var);
  P_.block<3, 3>(12, 12).diagonal().setConstant(params_.init_ba_var);
  r_eff_inflate_gnss_pos_ = 1.0;
  r_eff_inflate_gnss_vel_ = 1.0;
  r_eff_inflate_heading_yaw_ = 1.0;
}
...
{
  // 예측 단계: 상태/공분산 전파
  if (!initialized_ || !(dt_sec > 0.0) || !std::isfinite(dt_sec)) {
    return;
  }
  ...
  const Eigen::Vector3d omega = omega_base_radps - b_g_;
  const Eigen::Vector3d accel = accel_base_mps2 - b_a_;

  // Nominal propagation
  const Eigen::Quaterniond dq_body = delta_quat_from_dtheta(omega * dt_sec);
  q_map_from_base_ = (q_map_from_base_ * dq_body).normalized();

  const Eigen::Matrix3d R = q_map_from_base_.toRotationMatrix();
  const Eigen::Vector3d a_map = R * accel;

  p_map_ += v_map_ * dt_sec + 0.5 * a_map * dt_sec * dt_sec;
  v_map_ += a_map * dt_sec;
  ...
  Qc.block<3, 3>(0, 0).diagonal().setConstant(
    params_.gyro_noise_std *
    params_.gyro_noise_std);
  Qc.block<3, 3>(3, 3).diagonal().setConstant(
    params_.accel_noise_std *
    params_.accel_noise_std);
  Qc.block<3, 3>(6, 6).diagonal().setConstant(
    params_.gyro_bias_noise_std * params_.gyro_bias_noise_std);
  Qc.block<3, 3>(9, 9).diagonal().setConstant(
    params_.accel_bias_noise_std * params_.accel_bias_noise_std);
...
const char * EskfCore::check_large_correction(const V15 & dx) const
{
  ...
  if (std::isfinite(params_.max_correction_pos_m) &&
    params_.max_correction_pos_m > 0.0 &&
    std::isfinite(dp_norm) && dp_norm > params_.max_correction_pos_m)
  {
    return "large_correction_pos_skip";
  }
  if (std::isfinite(params_.max_correction_vel_mps) &&
    params_.max_correction_vel_mps > 0.0 &&
    std::isfinite(dv_norm) && dv_norm > params_.max_correction_vel_mps)
  {
    return "large_correction_vel_skip";
  }
  if (std::isfinite(params_.max_correction_att_rad) &&
    params_.max_correction_att_rad > 0.0 &&
    std::isfinite(dtheta_norm) &&
    dtheta_norm > params_.max_correction_att_rad)
  {
  ...
  const double target_factor)
{
  ...
  switch (channel) {
    case RInflateChannel::kGnssPos:
      state = &r_eff_inflate_gnss_pos_;
      break;
    case RInflateChannel::kGnssVel:
      state = &r_eff_inflate_gnss_vel_;
      break;
    case RInflateChannel::kHeadingYaw:
      state = &r_eff_inflate_heading_yaw_;
      break;
  ...
  const double tau_updates = params_.r_eff_decay_tau_updates;
  ...
  const RInflateChannel channel)
{
  ...
  const Eigen::Matrix<double, Dim, Dim> HPHT = H * P_ * H.transpose();
  ...
  if (std::isfinite(gate) && gate > 0.0 && std::isfinite(nis) && nis > gate) {
    if (!params_.nis_gate_inflate) {
      dbg.reason = "nis_gate_skip";
      return dbg;
    }
    target_factor = std::clamp(
      nis / gate, 1.0, std::max(1.0, params_.nis_gate_inflate_max));
  }
  ...
  const Eigen::Matrix<double, 15, Dim> K = P_ * H.transpose() * Sinv;
  if (!K.allFinite()) {
  ...
  const RInflateChannel channel)
{
  ...
  const double S_base = (H * P_ * H.transpose())(0, 0) + var;
  ...
  if (std::isfinite(gate) && gate > 0.0 && std::isfinite(nis) && nis > gate) {
    ...
    target_factor = std::clamp(
      std::sqrt(nis / gate), 1.0, std::max(1.0, params_.nis_gate_inflate_max));
  }

  const double factor_eff = smooth_r_eff_inflate(channel, target_factor);
  const double R_eff = var * factor_eff;
  const double S = (H * P_ * H.transpose())(0, 0) + R_eff;
  ...
  const Eigen::Matrix<double, 15, 1> K = P_ * H.transpose() * Sinv;
  if (!K.allFinite()) {
    dbg.reason = "K_nonfinite";
    return dbg;
  }
  ...
  const Eigen::Matrix3d & R)
{
  ...
  const Eigen::Vector3d r = z_p_map - p_map_;
  return update_vector_measurement<3, EskfGnssPosUpdateDebug>(
    H, r, R, params_.nis_gate_gnss_pos_3d, RInflateChannel::kGnssPos);
}
  ...
  const Eigen::Matrix3d & R)
{
  ...
  const Eigen::Vector3d r = z_v_map - v_map_;
  return update_vector_measurement<3, EskfGnssVelUpdateDebug>(
    H, r, R, params_.nis_gate_gnss_vel_3d, RInflateChannel::kGnssVel);
}
  ...
  const double yaw_var_rad2)
{
  ...
  return update_scalar_measurement(
    H, r, yaw_var_rad2,
    params_.nis_gate_heading_yaw, RInflateChannel::kHeadingYaw);
}
  ...
  const int axis, const double z_v_base_mps, const double var_mps2)
{
  ...
  const Eigen::Matrix3d R = q_map_from_base_.toRotationMatrix();
  const Eigen::Vector3d v_base = R.transpose() * v_map_;
  ...
  const double var_radps2)
{
  ...
  const double h = omega_meas_z_radps - b_g_.z();
...
void EskfCore::inject_and_reset(const V15 & dx)
{
  ...
  p_map_ += dx.segment<3>(0);
  v_map_ += dx.segment<3>(3);
  ...
  q_map_from_base_ = (q_map_from_base_ * dq).normalized();

  b_g_ += dx.segment<3>(9);
  b_a_ += dx.segment<3>(12);
  ...
  if (params_.use_so3_jacobian_reset) {
    const double angle = dtheta.norm();
    if (angle < 1e-8) {
