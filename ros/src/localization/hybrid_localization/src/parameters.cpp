...
void HybridLocalizationNodeParams::load(rclcpp::Node & node)
{
  ...
  const double tol_ms =
    node.declare_parameter("time_alignment_tolerance_ms", 100.0);
  ...
  vehicle_constraints.speed_factor = node.declare_parameter(
    "vehicle.speed_factor", vehicle_constraints.speed_factor);
